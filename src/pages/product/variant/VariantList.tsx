...
export default function VariantList({variants}: VariantListProps) {
    ...
    const [editForm, setEditForm] = useState({id: "", name: "", price: 0, cogs: 0})
    ...
    const onClickEditSave = (variant: Variant) => {
        console.log("Save edit variant : " + variant.id);
        setEditingId(null);
        setEditForm({id: "", name: "", price: 0, cogs: 0});
    ...
    return (
        <ListGroup.Item>
            <Container>
                                ...
                                <Col>
                                    <Form>
                                        ...
                                        <Form.Group as={Row} className="mb-3">
                                            ...
                                            <Col sm={9}>
                                                <Form.Control name="variantCount" value={editForm.price} onChange={handleChange}/>
                                            </Col>
                                        </Form.Group>
                                        <Form.Group as={Row} className="mb-3">
                                            <Form.Label column sm={3}>Harga Beli</Form.Label>
                                            <Col sm={9}><Form.Control name="variantCount" value={editForm.cogs} onChange={handleChange}/></Col>
                                        </Form.Group>
                                        <FormGroup as={Row} className="mb-3">
                                            <Col sm={{ span: 9, offset: 3 }} className="d-flex gap-2">
                                                <Button variant={"outline-primary"} onClick={() => onClickEditSave(variant)}>
                                                    Simpan<BsCheck2Circle/>
                                                </Button>
                                                <Button variant={"outline-secondary"} onClick={() => onClickEditCancel(variant)}>
                        ...
                        : <ListGroup.Item key={"variant-detail-row-" + variant.id}>
                            <Row>
                                <Col>
                                    ...
                                    <small className="text-muted">harga jual : <Currency value={variant.price}/></small> <br/>
                ...
                {Array.from({length: newVariantCount}, (_, index) => (
                    <VariantAddRow key={"variant-add-row" + index} />
                ))}
                <VariantAddButton
                    onClick={() => {
                        setNewVariantCount(newVariantCount + 1)
                    }}
                />
            </Container>
        </ListGroup.Item>
